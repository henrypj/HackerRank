...
import sys
"""
# Given an integer, N, traverse its digits d(1),d(2),...,d(n) and determine how
# many digits evenly divide N (i.e.: count the number of times N divided by each
# digit d(i) has a remainder of 0). Print the number of evenly divisible digits.
#
...
# by zero is undefined; thus, our count of evenly divisible digits is 3.
#
# Solution
"""
...
for a0 in range(t):
    ...
    for i in range(len(digits)):
        if (digits[i] != "0") and (n % int(digits[i]) == 0):
            numDigits += 1
    print(numDigits)
...
# The second line contains n space-separated integers (we'll refer to the i'th
# value as a(i)) describing the unique values of the set. 
#
# Constraints
#
# 1 <= n <= 10**5
...
#
# Explanation:
# The largest possible subset of integers is S' = {1,7,4}, because no two integers
# will have a sum that is evenly divisible by k = 3:
#
...
#
# The number 2 cannot be included in our subset because it will produce an 
# integer that is evenly divisible by k = 3 when summed with any of the other
# integers in our set:
#
...
n,k = [int(n),int(k)]
s = [int(arr_temp) for arr_temp in input().strip().split(' ')]
...
# This is first solution but fails on large ints
#subSize = 0
#for i in range(n-1):

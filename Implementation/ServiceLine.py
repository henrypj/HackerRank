...
# the highway. The length of the service lane is N units. The service lane 
...
# of the k'th segment of the service lane. It is guaranteed that while servicing
...
# 2. (4, 6): Here the largest allowed vehicle which can pass through the 5th 
#    segment is the car and for the 4th and 6th segment it's the truck. Hence
#    the largest vehicle allowed in these segments is a car.
# 3. (6, 7): In this example, the vehicle enters at the 6th segment and exits 
#    at the 7th segment. Both segments allow even trucks to pass through them.
#    Hence the answer is 3.
# 4. (3, 5): width[3] = width[5] = 2. While the 4th segment allows the truck,
#    the 3rd and 5th allow up to a car. So 2 will be the answer here.
# 5. (0, 7): The bike is the only vehicle which can pass through the 2nd segment,
...
N,T = [int(N),int(T)]
width = [int(arr_temp) for arr_temp in input().strip().split(' ')]
for a0 in range(T):
